...
-- Existing projects may still have the older status CHECK constraint.
-- Recreate it so a global NO can safely mark a candidate as rejected.
alter table public.hydrant_candidates
  drop constraint if exists hydrant_candidates_status_check;
alter table public.hydrant_candidates
  add constraint hydrant_candidates_status_check
