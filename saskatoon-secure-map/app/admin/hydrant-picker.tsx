...
async function loadAdminHydrantData() {
  ...
  if (cached?.features?.length) {
    // Admin should always see the current manual hydrants immediately, even
    // before pressing Publish Updates. The large City dataset stays cached.
    try {
      const manualResponse = await fetch('/api/admin/manual-hydrants', {
  ...
  // Replace the published manual portion with the admin working copy so the
  // picker shows edits immediately even before Publish Updates is pressed.
  try {
    const manualResponse = await fetch('/api/admin/manual-hydrants', {
      credentials: 'same-origin',
      cache: 'no-store',
    ...
    if (manualResponse.ok) {
      const manualData = await manualResponse.json()
      const city = (Array.isArray(data.features) ? data.features : []).filter((feature: any) => !isManualHydrant(feature))
      const manual = Array.isArray(manualData?.features) ? manualData.features : []
      const updated = makeHydrantBundle(city, manual)
      await writeHydrantCache(updated)
      return updated
