...
export default function HydrantPicker({ idPrefix = 'hydrant' }: { idPrefix?: string }) {
    ...
    <>
      ...
      <div className="coordinate-grid">
        <div>
          ...
          <input id={`${idPrefix}-lat`} name="latitude" value={lat} readOnly required placeholder="Tap map" />
        </div>
        <div>
          <label htmlFor={`${idPrefix}-lng`}>Longitude</label>
          <input id={`${idPrefix}-lng`} name="longitude" value={lng} readOnly required placeholder="Tap map" />
        </div>
      </div>

      <div>
        <label htmlFor={`${idPrefix}-address`}>Address / location name</label>
        <input id={`${idPrefix}-address`} name="address" placeholder="Example: 123 Main St or beside alley" maxLength={160} />
      </div>

      <div>
        <label htmlFor={`${idPrefix}-note`}>Note (optional)</label>
        <input id={`${idPrefix}-note`} name="note" placeholder="Example: New hydrant not shown by City data" maxLength={300} />
      </div>
    </>
  )
}
