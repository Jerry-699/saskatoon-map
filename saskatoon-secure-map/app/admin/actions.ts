...
export async function publishMapUpdates() {
  const { supabase } = await requireAdminWithUser()

  const { data: nextVersion, error } = await supabase.rpc('publish_map_data')
  if (error) {
    console.error('Publish map updates failed:', error)
    redirect('/admin?message=' + encodeURIComponent('Publish failed: ' + error.message))
  }

  revalidatePath('/admin')
  redirect('/admin?message=' + encodeURIComponent(`All route and hydrant changes are now published to users (version ${nextVersion}).`))
}
