...
export async function publishHydrantUpdate() {
  const { supabase } = await requireAdminWithUser()
  ...
  if (readError) {
    redirect('/admin?message=' + encodeURIComponent('Run supabase/hydrant-cache-version.sql first. ' + readError.message))
  }

  const nextVersion = Number(current?.value || 1) + 1
  const { error } = await supabase
    .from('app_settings')
  ...
  revalidatePath('/admin')
  redirect('/admin?message=' + encodeURIComponent(`Hydrant update published. Devices will refresh once when they next load hydrants (version ${nextVersion}).`))
}
