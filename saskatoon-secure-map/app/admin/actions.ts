...
export async function addHydrant(formData: FormData) {
  ...
  if (!inSaskatoonArea) {
    redirect('/admin?message=' + encodeURIComponent('Pick a valid Saskatoon hydrant location on the map.'))
  }
  ...
  revalidatePath('/admin')
  redirect('/admin?message=' + encodeURIComponent('Fire hydrant added to the map.'))
}
...
export async function addHydrantCandidate(formData: FormData) {
  const { supabase, user } = await requireAdminWithUser()
  ...
  if (!inSaskatoonArea) {
    redirect('/admin?message=' + encodeURIComponent('Pick a valid Saskatoon candidate location on the map.'))
  }
  ...
  revalidatePath('/admin')
  redirect('/admin?message=' + encodeURIComponent('Unverified hydrant candidate added. Approved users can now confirm it.'))
}

export async function deleteHydrantCandidate(formData: FormData) {
  const { supabase } = await requireAdminWithUser()
  ...
  revalidatePath('/admin')
  redirect('/admin?message=' + encodeURIComponent('Hydrant candidate deleted.'))
}
