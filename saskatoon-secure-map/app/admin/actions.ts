...
export async function revokeUser(formData: FormData) {
  ...
  await supabase
    .from('profiles')
    // Also clear blocked so the user returns to a clean "Pending" state,
    // keeping the three-state machine (pending / approved / blocked) unambiguous.
    .update({ approved: false, blocked: false })
    .eq('id', id)
