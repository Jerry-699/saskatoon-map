...
async function loadAllCityHydrants(): Promise<GeoJsonFeature[]> {
  ...
  const rawIds: unknown[] = Array.isArray(idJson?.objectIds) ? idJson.objectIds : []
  const ids: number[] = Array.from(
    new Set<number>(rawIds.map((id) => Number(id)).filter((id): id is number => Number.isFinite(id)))
...
export async function GET() {
  ...
  const manualPromise = supabase
    .from('published_manual_hydrants')
