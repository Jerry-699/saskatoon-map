...
## Private driven routes + address directions
...
- Address search uses a server-side OpenStreetMap Nominatim lookup restricted to Saskatoon and OSRM driving directions.
- Public geocoding/routing services are suitable for light use and have no guaranteed SLA; use a dedicated provider for heavy/mission-critical use.

## Voice navigation
- Address search has a microphone button using the browser Speech Recognition API (`SpeechRecognition` / `webkitSpeechRecognition`).
- Spoken turn guidance uses the browser Speech Synthesis API and prefers an English-Canada voice when available.
- iPhone/iPad: Safari can use browser speech features when supported and microphone permission is granted. If speech recognition is unavailable on a particular iOS/Safari version, focus the address box and use the microphone on the iPhone keyboard as a fallback.
- The 🔊 Voice ON/OFF setting is remembered on the device.
...
- Multi-route reference selection: show several saved routes at once and toggle old routes while a new route is recording, without stopping the recording.
